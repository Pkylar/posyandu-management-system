...
const COLORS = {
  bgHeader: "linear-gradient(135deg,rgb(53, 20, 219) 0%,rgb(64, 171, 234) 100%)",
...
export default function Header({
  ...
  const handleMenuClick = (item: MenuItem) => {
    ...
    if (item.isLogout) {
      // Eksekusi proses logout - konsisten dengan AuthGuard
      localStorage.removeItem("is_logged_in");
      localStorage.removeItem("access_token");
      localStorage.removeItem("token_type");
      localStorage.removeItem("user");
      // Keep user registration data for next login
      // localStorage.removeItem("registered_users"); // Don't remove this
      router.push("/login");
    } else if (item.url) {
